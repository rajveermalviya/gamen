...
Resulting binaries shouldn't require any dependency to be installed by the users.

But developers will need some `devel` packages.
...
android backend uses [`game-activity`](https://developer.android.com/games/agdk/game-activity), to help with dependency resolution and building apk from Go source [`tsukuru`](https://github.com/rajveermalviya/tsukuru) is used.
...
`gamen`'s api is a mix of [`glfw`](https://github.com/glfw/glfw/) and [`winit`](https://github.com/rust-windowing/winit) and some terminologies from Wayland.
